  ...
  <v-card class="mb-4" :class="priorityClass" elevation="2" rounded="lg">
    <v-card-title class="d-flex justify-space-between align-center py-2">
      ...
      <v-tooltip location="top">
        ...
        <span
          >{{ $t("task.priority") }}:
          {{ $t(`task.priorities.${task.priority.toLowerCase()}`) }}</span
        >
      </v-tooltip>
    </v-card-title>
    ...
    <v-card-actions class="pa-3">
      ...
      <v-chip small :color="statusColor" class="white--text">
        {{ $t(`task.statuses.${task.status.replace(/\s/g, "").toLowerCase()}`) }}
      </v-chip>
    </v-card-actions>
  </v-card>
</template>
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const formatDate = (date: string) => {
      const parsedDate = parse(date, "dd-MM-yyyy", new Date());
      return parsedDate.toLocaleDateString(t("locale"), {
        year: "numeric",
        month: "short",
        day: "numeric",
      });
...
export default defineComponent({
  ...
  setup(props) {
    ...
    // Fix for translation key issue
    const statusKey = computed(() => {
      // Convert status to camelCase format to match the JSON keys
      const formattedKey = props.task.status
        .toLowerCase()
        .replace(/\s+(.)/g, (_, c) => c.toUpperCase()); // Converts "in progress" -> "inProgress"
    ...
    return {
      ...
      priorityClass,
      statusColor,
      statusKey, // Ensure the corrected key is used in template
      formatDate,
      t,
    };
  },
